...
import { CheckoutDto } from 'src/app/_dto/checkoutDto.model';
...
deletedNumber:number;
 ...
 tableSize:number=3;
  ...
  getBaskets(){
    ...
    deletebyUserone(basketId:number,cartId:number,productId:number,adet:number){
         var basketDeleteOneDto=new BasketDeleteOneDto();
        basketDeleteOneDto.basketId=basketId;
        basketDeleteOneDto.cartId=cartId;
        basketDeleteOneDto.productId=productId;
        basketDeleteOneDto.adet=adet;
    ...
    checkout(personId:number){
      ...
      this.checkoutDto.personId=personId;


// this.basketService.checkout(this.checkoutDto).subscribe((success)=>{
// this.spinnerService.show();
...
//this.deletedNumber=personId;
      this.spinnerService.show();
      this.basketService.checkout(this.checkoutDto).subscribe((success)=>{
        this.spinnerService.hide();


        this.basketService.deleteUserAllBasket(this.checkoutDto.personId).subscribe(()=>{

              this.getBaskets();

        },(err)=>{

          this.toastr.error("Sepet Silinirken hata oluştu.")
        })
    ...
    exportExel(){
      let element = document.getElementById("sepet-exel");

      const ws:XLSX.WorkSheet=XLSX.utils.table_to_sheet(element);
      const wb:XLSX.WorkBook=XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb,ws,'Sepet');

      let title="Sepet.xlsx";
